...
* [`src/visualizeml2/`](src/visualizeml2/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/visualizeml2/core.cljs): contains the SPA entry point, `init`
...
5. Clone this repo and open a terminal in the `visualizeml2` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/visualizeml2/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns visualizeml2.example
  (:require [visualizeml2.config :as config])
